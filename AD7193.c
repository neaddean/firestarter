...
unsigned long AD7193_Init(void){
    AD7193_Reset();
    long mode = MODE_CONT_CONV | MODE_NO_AVG | MODE_INT_CLK1 | MODE_FILTER_RATE(5) | MODE_DAT_STA;
    long config = CONFIG_REFSEL_1 | CONFIG_CHANNEL_SEL(1) | CONFIG_CHANNEL_SEL(2) | CONFIG_REFDET | CONFIG_BUF | CONFIG_GAIN_64;
  //  long config1 = CONFIG_REFSEL_1 | CONFIG_CHANNEL_SEL(2) | CONFIG_REFDET | CONFIG_BUF | CONFIG_GAIN_64;
    AD7193_WriteReg(AD7193_MODE_REG,mode,3);
    AD7193_WriteReg(AD7193_CONFIG_REG,config,3);
  //  AD7193_WriteReg(AD7193_CONFIG_REG,config1,3);
